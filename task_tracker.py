...
def display_task_table(tasks, show_completed=False):
    ...
    for idx, task in enumerate(tasks):
        ...
        # Use unique key combining task_id and index
        unique_key = f"{task['id']}_{idx}"
        
        col1, col2, col3, col4, col5 = st.columns([0.5, 3, 1.5, 1, 1])
        
        with col1:
            checkbox_key = f"check_{unique_key}"
            if st.checkbox("✓", value=task['completed'], key=checkbox_key, label_visibility="collapsed"):
                if not task['completed']:  # Only toggle if not already completed
                    toggle_task_completion(task['id'])
                    st.rerun()
            elif task['completed']:  # User unchecked a completed task
                toggle_task_completion(task['id'])
                st.rerun()
